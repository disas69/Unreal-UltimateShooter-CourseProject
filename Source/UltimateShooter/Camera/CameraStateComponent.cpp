...
{
    const FCameraStateConfig* CameraStateConfig =
        CameraStates.FindByPredicate([State](const FCameraStateConfig& Config) { return Config.CameraState == State; });
...
void UCameraStateComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    if (CameraStateConfig != nullptr && Character != nullptr)
    {
        const FVector CameraCurrentOffset =
            FMath::VInterpTo(Character->GetCameraBoom()->SocketOffset, CameraStateConfig->CameraOffset, DeltaTime, CameraTransitionSpeed);
        Character->GetCameraBoom()->SocketOffset = CameraCurrentOffset;

        const float CameraCurrentFOV =
            FMath::FInterpTo(Character->GetFollowCamera()->FieldOfView, CameraStateConfig->CameraFOV, DeltaTime, CameraTransitionSpeed);
