...
void UWeaponHandlerComponent::FireWeapon()
{
	...
	if (BarrelSocket != nullptr)
	{
		...
		{
			UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), WeaponData.ImpactParticle, FireWeaponHitResult.Location, FireWeaponHitResult.ImpactNormal.Rotation());
		}
		
		if (WeaponData.TrailParticle != nullptr)
		{
			UParticleSystemComponent* Trail = UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), WeaponData.TrailParticle, SocketLocation, (FireWeaponTraceEndLocation - SocketLocation).Rotation());
			Trail->SetVectorParameter("Target", FireWeaponTraceEndLocation);
		}
		
		if (WeaponData.FireFX != nullptr)
		{
			UGameplayStatics::SpawnEmitterAttached(WeaponData.FireFX, Character->GetMesh(), BarrelSocket->GetFName(), SocketLocation, SocketTransform.GetRotation().Rotator(), EAttachLocation::KeepWorldPosition);
		}
	}
}
